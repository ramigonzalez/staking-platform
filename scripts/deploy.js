...
// Contract to deploy
const contractToDeploy = 'MyContract';

async function main() {
    // Get provider. Connect with default hardhat provider
    ...
    const contractABIPath =
        path.resolve(process.cwd(), 'artifacts/contracts/', contractToDeploy) + '.sol/' + contractToDeploy + '.json';
    ...
    if (contractVersion != 100) {
        ...
        console.log('-- Contract version:\t', parseInt(contractVersion));
        console.log('---------------------------------------------------------------------------------------');
